...
func main() {
	...
	ChiaCmd := strings.Join([]string{ChiaExec, "plots", "create", "-n", confYaml.NumPlots, "-k", confYaml.KSize, "-b", confYaml.Buffer, "-r", confYaml.Threads, "-f", farmKey, "-p", poolKey, "-t", confYaml.TempPath, "-d", confYaml.FinalPath[0]}, " ")
	StartPlots(ChiaCmd, LogPath, *confYaml)
...
func StartPlots(ChiaCmd, LogPath string, confYaml Config) {
